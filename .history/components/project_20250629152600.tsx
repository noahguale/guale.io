...
export const RecentProjects = ({
	...
	const recentProjects = projects
		.filter((project) => new Date(project.date) >= cutoffDate)
...
export const ProjectsByYear = ({
	...
	const yearProjects = projects
		.filter((project) => new Date(project.date).getFullYear() === year)
		.sort((a, b) => new Date(b.date).getTime() - new Date(a.date).getTime())
...
export const ProjectsByDateRange = ({
	projects,
	startDate,
	endDate,
}: {
...
export const ProjectsSortedByDate = ({
	projects,
	order = 'desc',
}: {
	projects: ProjectProps[]
...
// Utility function to get all unique years from projects
export const getProjectYears = (projects: ProjectProps[]): number[] => {
...
// Combined filters - featured AND recent
export const FeaturedRecentProjects = ({
	projects,
	months = 12,
}: {
	projects: ProjectProps[]
	...
	const featuredRecentProjects = projects
		.filter(
			(project) =>
				project.featured && new Date(project.date) >= cutoffDate,
		)
		.sort((a, b) => new Date(b.date).getTime() - new Date(a.date).getTime())

	return (
		<div className='flex flex-col gap-4 md:flex-row'>
			{featuredRecentProjects.map((project) => (
				<Project key={project.title} {...project} />
			))}
		</div>
	)
