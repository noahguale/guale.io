...
2. Learn how build scaleable products in a short time frame (just like what Theo did)
...
1. **Web Sockets**: Two-way communication with more complex, manual reconnection handling requirements.
2. **Server-Sent Events (SSE)**: One-way connection from server to client. For a chat app, only the server needs the connection to the client in order to send chunks of data back to the user.
3. **Sync Engine**: Keeping information up to date and managing the differences between client state and server state.
4. **Stream**: Continuous flow of data chunks that includes the AI Model Stream (raw response from the AI server in chunks), SSE (Server-Sent Events for sending chunks to the user), and Database integration (server sends chunks to Convex database).

## Overview

<img
  src="/images/blog/merkato-flow.png"
...
1. **Stay focused on the original plan.** I got way too caught up trying to perfect the streaming feature instead of building out the full MVP. Should have moved on and circled back if time allowed.
2. **Be more confident in my skills.** The final product proved I could build sophisticated applications under pressure. I had the technical chops all along but kept second-guessing myself.
3. **Join more hackathons and get my work out there.** This experience showed I can compete with talented developers and deliver meaningful results. I need to stop hiding behind side projects and start showcasing what I can actually build.
